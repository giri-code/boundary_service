...
async def verify_internal_token(request: Request):
    ...
    if not settings.INTERNAL_API_KEY:
        logger.warning("INTERNAL_API_KEY is not set in configuration. Rejecting request.")
        raise HTTPException(
    ...
    if not secrets.compare_digest(token.encode('utf-8'), settings.INTERNAL_API_KEY.encode('utf-8')):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
