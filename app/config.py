...
class Settings:
    ...
    # ── Service Information ────────────────────────────────────────────────────
    APP_NAME: str = field(default_factory=lambda: os.getenv("APP_NAME", "Object Boundary Detection Service"))
    ...
    STORAGE_BACKEND: str = field(default_factory=lambda: os.getenv("STORAGE_BACKEND", "auto"))
    LOCAL_STORAGE_ROOT: str = field(
        default_factory=lambda: os.getenv("LOCAL_STORAGE_ROOT", str(BASE_DIR / "storage"))
    )
    ...
    ENABLE_ROI_CROPPING: bool = field(
        default_factory=lambda: os.getenv("ENABLE_ROI_CROPPING", "True").lower() in ("true", "1")
    )
